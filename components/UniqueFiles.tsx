        ...
        <div className="grid sm:grid-cols-2 w-full sm:gap-x-4 lg:gap-x-10 desktop:gap-x-20 2xl:gap-x-0 gap-y-4 sm:gap-y-0 md:gap-y-6 lg:gap-y-0 desktop:gap-y-8 largesceen:gap-y-10  place-items-center">
          {Unique.map((item, idx: number) => (
            <div
              ...
              <h1 className="text-white font-semibold text-sm sm:text-base lg:text-lg  xl:text-[1.375rem] largesceen:text-[1.7rem] mb-1 md:mb-4 lg:mb-6 xl:mb-9 largesceen:mb-10 w-fit  largesceen:w-full">
                {item.title}
              </h1>

              <p className="opacity-80 text-white text-xs   xl:text-lg desktop:text-xl largesceen:text-2xl w-[16rem] md:w-[20rem] lg:w-[25rem] xl:w-[30rem] desktop:w-[33.75rem] largesceen:w-[40rem]">
                {item.text}
              </p>
              ...
              <Image
                src={item.Img}
                alt={item.title}
                className="w-8 md:w-10 lg:w-12 xl:w-16 desktop:w-[4rem]  largesceen:w-[4.5rem] absolute top-0 right-6 sm:right-4 md:right-4 lg:right-8 xl:right-6 desktop:right-8 largesceen:right-[3rem] label"
              />
            </div>
          ))}
        </div>
